...
# =========================
# Step 3. 调用 Ollama LLM 命名类别
# =========================
def call_ollama_llm(prompt: str, timeout: int = 60) -> str:
    """调用本地 Ollama 模型"""
    ...
    try:
        ...
        logging.error(f"Ollama request timeout ({timeout}s)")
        return ""
    except Exception as e:
...
def name_cluster_with_llm(type_name: str, dataset: str, sample_texts: List[str]) -> str:
    """调用 Ollama LLM 生成聚类名称"""
    joined = "\n".join(sample_texts[:10])  # 只取前10个示例
    
    prompt = f"""You are a finance data classifier.
...
Task: Propose a short, meaningful lowercase name (1-3 words) for this group.
Examples: "momentum", "valuation_ratio", "sentiment_score", "earnings_quality"

Return ONLY the name (no explanation):"""
    
    name = call_ollama_llm(prompt)
    
    if not name:
        # 如果 Ollama 失败，使用默认命名
        name = f"{dataset}_{type_name}".lower()
        logging.warning(f"Ollama failed, using default name: {name}")
    
    # 清理非法字符
    name = name.replace(" ", "_").replace("-", "_").replace('"', '').replace("'", "").lower()
    # 只保留第一行（防止模型返回多行）
    name = name.split('\n')[0].strip()
...
def generate_template_fields_v2():
    ...
    for (dataset, dtype), subdf in grouped:
        ...
        for cluster_id, field_ids in clusters.items():
            ...
            type_name = name_cluster_with_llm(dtype, dataset, sample_texts)
