...
class AdaptiveAlphaMiner:
    ...
    def _get_data_fields_for_region(self, temp_generator, delay: int = 1) -> List[Dict]:
        ...
        try:
            ...
            # First, get available datasets for this region (all categories)
            datasets_params = {
                # Remove 'category' restriction to get all datasets
                'delay': delay,
                'instrumentType': 'EQUITY',
                'region': self.selected_region,
                'universe': self.selected_universe,
                'limit': 200  # Increase limit to get more datasets
            }
            
            logger.info(f"Getting available datasets for region {self.selected_region} (all categories)")
